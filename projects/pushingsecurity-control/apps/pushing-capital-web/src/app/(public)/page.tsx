    ...
    <div className="container mx-auto px-6 py-4 flex justify-between items-center backdrop-blur-md bg-black/30 border-b border-gray-800/50">
      <div className="flex items-center">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-3">
            <path d="M12 2L2 7L12 12L22 7L12 2Z" stroke="#00FFAA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            <path d="M2 17L12 22L22 17" stroke="#00FFAA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            <path d="M2 12L12 17L22 12" stroke="#00FFAA" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
        <span className="text-xl font-bold tracking-widest text-gray-100">PUSHING CAPITAL</span>
      </div>
      <nav className="hidden md:flex items-center space-x-8">
        <a href="#verticals" className="font-mono text-sm hover:text-[#00FFAA] transition-colors duration-300">Verticals</a>
        <a href="#intel" className="font-mono text-sm hover:text-[#00FFAA] transition-colors duration-300">Intel</a>
        <a href="#architect" className="font-mono text-sm hover:text-[#00FFAA] transition-colors duration-300">Architects</a>
      </nav>
      <div className="hidden md:block">
        <button className="font-mono text-sm border border-gray-600 px-4 py-2 rounded-sm hover:border-[#00FFAA] hover:text-[#00FFAA] transition-colors duration-300">
          Secure Login
        </button>
      </div>
...
const MobileNav = ({ isOpen }: any) => (
  <div className={`fixed top-0 left-0 w-full h-full bg-black z-40 transform ${isOpen ? 'translate-y-0' : '-translate-y-full'} transition-transform duration-300 ease-in-out md:hidden`}>
    <div className="container mx-auto px-6 pt-24 flex flex-col items-center space-y-8">
        ...
        <button className="font-mono text-lg border border-[#00FFAA] text-[#00FFAA] px-6 py-3 rounded-sm mt-8">
          Secure Login
        </button>
    </div>
  </div>
...
const HeroSection = () => (
    <section className="relative min-h-screen flex items-center justify-center text-center overflow-hidden pt-20">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <h1 className="text-5xl md:text-8xl font-extrabold tracking-tighter text-gray-100 mb-4">
          ARCHITECTING ALPHA
        </h1>
        <p className="font-mono text-lg md:text-xl text-gray-400 max-w-3xl mx-auto mb-8">
          Precision capital deployment in high-growth verticals. We build, fund, and scale the infrastructure of tomorrow.
        </p>
        <div className="flex justify-center items-center space-x-4">
          <button className="group relative inline-flex items-center justify-center bg-[#00FFAA] text-black font-bold py-3 px-8 rounded-sm overflow-hidden transition-all duration-300">
            <span className="relative z-10 flex items-center">
              Explore Mandates <ArrowRight className="ml-2 h-5 w-5 transition-transform duration-300 group-hover:translate-x-1" />
            </span>
          </button>
        </div>
      </div>
...
const ServiceCard = ({ icon, title, description }: any) => (
    <div className="bg-gray-950/50 border border-gray-800 p-8 rounded-sm backdrop-blur-sm transition-all duration-300 hover:border-[#00FFAA]/50 hover:-translate-y-1">
        <div className="mb-4 text-[#00FFAA]">
            {icon}
        </div>
